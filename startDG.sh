...
cat << EOF >> /opt/oracle/oradata/dbconfig/${ORACLE_SID}/listener.ora
...
sed -in '/localhost\:1521/d' /opt/oracle/oradata/dbconfig/${ORACLE_SID}/tnsnames.ora

# Add the TNS entries for all databases in the cluster:
cat /opt/oracle/scripts/tnsnames.ora >> /opt/oracle/oradata/dbconfig/${ORACLE_SID}/tnsnames.ora
...
cat << EOF > /opt/oracle/oradata/dbconfig/${ORACLE_SID}/initDG.ora
*.db_name='${ORACLE_SID}'
EOF
